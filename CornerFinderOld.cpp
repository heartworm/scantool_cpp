	...
	for (Vec4i &lineA : hLines) {
		for (Vec4i &lineB : hLines) {
			if (&lineA != &lineB) {
				...
				double ang = fabs(constrainAngle(angle(i1, j1, i2, j2)));
...
bool CornerFinder::Cluster::inBounds(cv::Point2f p1, cv::Point2f p2) {
	int dx = std::fabs(p2.x - p1.x);
	int dy = std::fabs(p2.y - p1.y);
...
double CornerFinder::angle(int i1, int j1, int i2, int j2) {
	return acos(((i1 * i2) + (j1 * j2)) / (sqrt(i1 * i1 + j1 * j1)*sqrt(i2 * i2 + j2 * j2)));
}

double CornerFinder::constrainAngle(double x) {
	x = fmod(x + 180, 360);
	if (x < 0)
		x += 360;
	return x - 180;
...
void CornerFinder::clockwiseSort(cv::Point2f(&corners)[4]) {
