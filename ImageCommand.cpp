...
pp::VarDictionary ImageCommand::getOutput() {
	...
	} else if (command == "deskew") {
		cv::Mat deskewMat = KeystoneProcessor::process(image, corners);

		uint32_t bytes = deskewMat.total() * deskewMat.elemSize();
		pp::VarArrayBuffer deskewVar(bytes);

		void* srcData = deskewMat.data;
		void* dstData = deskewVar.Map();
		memcpy(dstData, srcData, bytes);
		
		pp::VarDictionary imgDict;
		imgDict.Set("number", number);
		imgDict.Set("id", id);
		imgDict.Set("deskewStatus", "SUCCESS");
		imgDict.Set("deskewData", deskewVar);
		imgDict.Set("deskewHeight", deskewMat.rows);
		imgDict.Set("deskewWidth", deskewMat.cols);
